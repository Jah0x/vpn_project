...
## 2025-07-16
- Улучшен скрипт `update_zerologsvpn.sh`: добавлена проверка наличия `pnpm`,
  сброс локальных изменений через `git reset --hard`,
  выбор между `docker compose` и `docker-compose`.
- Документация по установке обновлена, лог обновления теперь содержит точное время.
